...
export default async function QianaiPostPage({
  params,
}: {
  ...
  if (
    !post ||
    post.section !== PostSection.QIANAI ||
    ...
    <article className="stack flower-article-shell">
      <section className="flower-article article-card">
        <div className="article-actions">
          <Link className="button secondary" href="/qianai">
            返回列表
          </Link>
          {canManage ? (
            <Link className="button" href={`/qianai/${post.id}/edit`}>
              编辑文章
            </Link>
          ) : null}
        </div>
